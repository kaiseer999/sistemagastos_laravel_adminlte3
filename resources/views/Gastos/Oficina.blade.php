...
@section('title', 'Gastos oficina')

@section('content_header')
    <h1 class="m-0 text-dark">Gastos oficina</h1>
@stop
...
@section('content')
    <div class="card">
        <div class="card-body">
            ...
            @foreach($gastosOficina as $gasto)
            @php
            $btnEdit = '<a href="' . url('gastosoficina/' . $gasto->id . '/edit') . '" class="btn btn-xs btn-default text-primary mx-1 shadow" title="Edit"><i class="fa fa-lg fa-fw fa-pen"></i></a>';
            $btnDelete =   '<form action="' .
                            url('gastosoficina/' . $gasto->id) .
                            '" method="POST">
                ' .
                csrf_field() .
                '
            ...
            bordered compressed with-buttons beautify>
                @foreach($gastosOficina as $gasto)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('gastosoficina.show', ['gastosoficina' => $gasto->id]) }}">
