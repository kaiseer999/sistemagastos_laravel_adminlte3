...
@section('title', 'Editar gastos personales')
@section('content_header')

    <h1 class="m-0 text-dark">Editar un gasto personal</h1>
@stop

@section('content')

<form action="{{ url('/gastospersonales/' . $gastoPersonal->id) }}" method="POST" enctype="multipart/form-data">
    @csrf
    {{ method_field('PATCH') }}

    <div class="card">
        <div class="card-header" style="background-color: #0202f5; color: #ffffff;">
            ...
            <div class="row">
                <div class="col-md-6">
                    <x-adminlte-select2 name="mes" id="mes" placeholder="Seleccione un mes..." label="Mes de pago"
                        ...
                        @foreach ($nombresMeses as $index => $nombreMes)
                            <option value="{{ $index + 1 }}" {{ $index + 1 == $gastoPersonal->mes ? 'selected' : '' }}>
                                {{ $nombreMes }}
                            </option>
                        @endforeach
                    </x-adminlte-select2>
                ...
                <div class="col-md-6">
                    <x-adminlte-select2 name="compromiso_pago_id" id="compromiso_pago_id"
                        ...
                        @foreach ($datosCompromisosdePago as $compromiso)
                            <option value="{{ $compromiso->id }}" {{ $compromiso->id == $gastoPersonal->compromiso_pago_id ? 'selected' : '' }}>
                ...
                <div class="col-md-6">
                    ...
                    <x-adminlte-input-date name="fecha_de_pago" id="fecha_de_pago" :config="$config"
                        ...
                        value="{{ $gastoPersonal->fecha_de_pago }}">
                ...
                <div class="col-md-6">
                    <x-adminlte-input name="valor_de_pago" id="valor_de_pago" label="Valor"
                        ...
                        value="{{ $gastoPersonal->valor_de_pago }}">
                ...
                <div class="col-md-12">
                    <label><b>¡Importante!</b> Si deseas modificar o editar algún archivo, ¡adelante! 
                        Sube archivos solo si es necesario para la actualización. La previsualización no 
                        está disponible en esta vista. 🌟</label>
