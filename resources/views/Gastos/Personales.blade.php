...
@section('title', 'Gastos personales')

@section('content_header')
    <h1 class="m-0 text-dark">Gastos personales</h1>
@stop
...
@section('content')
   <div class="card">
    <div class="card-body">
        ...
        @foreach($gastosPersonales as $gasto)
        @php
        $btnEdit = '<a href="' . url('gastospersonales/' . $gasto->id . '/edit') . '" class="btn btn-xs btn-default text-primary mx-1 shadow" title="Edit"><i class="fa fa-lg fa-fw fa-pen"></i></a>';
        $btnDelete =   '<form action="' .
                        url('gastospersonales/' . $gasto->id) .
                        '" method="POST">
            ' .
            csrf_field() .
            '
        ...
        bordered compressed with-buttons beautify>
            @foreach($gastosPersonales as $gasto)
                <tr>
                    ...
                    <td>
                        <a href="{{ route('gastospersonales.show', ['gastospersonale' => $gasto->id]) }}">
