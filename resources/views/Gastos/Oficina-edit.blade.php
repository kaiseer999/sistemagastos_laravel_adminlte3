...
@section('title', 'Editar gastos Oficina')
@section('content_header')

    <h1 class="m-0 text-dark">Editar un gasto oficina</h1>
@stop

@section('content')

<form action="{{ url('/gastosoficina/' . $gastosOficina->id) }}" method="POST" enctype="multipart/form-data">
    @csrf
    {{ method_field('PATCH') }}

    <div class="card">
        <div class="card-header" style="background-color: #56017d; color: #ffffff;">
            ...
            <div class="row">
                <div class="col-md-6">
                    <x-adminlte-select2 name="mes" id="mes" placeholder="Seleccione un mes..." label="Mes de pago"
                        ...
                        @foreach ($nombresMeses as $index => $nombreMes)
                            <option value="{{ $index + 1 }}" {{ $index + 1 == $gastosOficina->mes ? 'selected' : '' }}>
                                {{ $nombreMes }}
                            </option>
                        @endforeach
                    </x-adminlte-select2>
                ...
                <div class="col-md-6">
                    <x-adminlte-select2 name="compromiso_pago_id" id="compromiso_pago_id"
                        ...
                        @foreach ($datosCompromisosdePago as $compromiso)
                            <option value="{{ $compromiso->id }}" {{ $compromiso->id == $gastosOficina->compromiso_pago_id ? 'selected' : '' }}>
                ...
                <div class="col-md-6">
                    ...
                    <x-adminlte-input-date name="fecha_de_pago" id="fecha_de_pago" :config="$config"
                        ...
                        value="{{ $gastosOficina->fecha_de_pago }}">
                ...
                <div class="col-md-6">
                    <x-adminlte-input name="valor_de_pago" id="valor_de_pago" label="Valor"
                        ...
                        value="{{ $gastosOficina->valor_de_pago }}">
